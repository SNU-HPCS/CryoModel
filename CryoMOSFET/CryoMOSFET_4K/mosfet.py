...
class mosfet:
    ...
    def get_intrinsic_carrier_density (self, temperature):
        ...
        # due to the precision limit of Python
        if (temperature < 10): 
            ni = ncnv_sqrt * exp (-1 * self.bandgap (10)/(2*self.BOLTZMANN*10))
        return ni
    ...
    def get_vth_on (self, temperature, Vdd):
        ...
        V_shift_sch = -0.5 * self.variables.dvt0 / (cosh (self.variables.dvt1 * self.variables.leff / lt0) - 1) * 2*(self.BOLTZMANN * temperature * (log (self.variables.ndep * self.variables.nsd)- 2*log (ni)) - surface_potential)
    ...
    def calc_currents (self):
        ...
        self.Igate = igd + igs + igc0 + igbacc
