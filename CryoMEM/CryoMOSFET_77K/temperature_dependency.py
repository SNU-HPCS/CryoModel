...
class temperature_dependency:
    ...
    def vth0_model (self, temperature):
        vth0_300k_ref = 0.5
        vth0_77k_ref = 0.6
        Vth0_temperature = ((vth0_300k_ref - vth0_77k_ref) / 223 * (temperature - 300) + vth0_300k_ref)/vth0_300k_ref;
    ...
    def u0_model (self, temperature, mos_type, variables):
        ...
        leffs = [14, 20, 26, 100, 120, 1000, 10000]
        u0n_cryo = [122.23, 164.74, 209.92, 521.7, 706.82, 843.22, 941.54]
        u0p_cryo = [32.54, 32.54, 50.58, 105.88, 177.65, 274.65, 361.18]
        u0n = [69.97, 84.14, 110.72, 228.52, 275.47, 317.98, 310.01]
        u0p = [19.6, 22.35, 30.19, 57.25, 84.31, 112.54, 129.02]
        ...
        if leff in leffs:
            ...
            if mos_type == 1:
                u0_77k = u0n_cryo [index_]
                u0_300k = u0n [index_]
            elif mos_type == 2:
                u0_77k = u0p_cryo [index_]
                u0_300k = u0p [index_]
        ...
        # we conduct linear interpolation to find u0_300k,77k
            ...
            if mos_type == 1:
                u0_77k = np.average ([u0n_cryo[low_index], u0n_cryo[high_index]], weights=weight_)
                u0_300k = np.average ([u0n[low_index], u0n[high_index]], weights=weight_)
            elif mos_type == 2:
                u0_77k = np.average ([u0p_cryo[low_index], u0p_cryo[high_index]], weights=weight_)
        ...
        weight_ = [300-temperature, temperature-77]
        weight_ = [i/sum(weight_) for i in weight_]
        u0_temperature = np.average ([u0_77k, u0_300k], weights=weight_)/u0_300k
